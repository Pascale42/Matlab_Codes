...
%
% function CheckEegStates(FileBase,State,FreqRange,Channels,Window, Action, Overwrite)
%
...
[State,FreqRange,Channels,Window,Action, Overwrite] = ...
    DefaultArgs(varargin,{[],[1 100],[],4,'display',0});
...
if ~FileExists([FileBase '.eegseg.mat']) || Compute==1
    ...
    if isempty(Channels)
        ...
        Channels = str2num(ch{1}); %#ok<ST2NM>
    end
    ...
    elseif FileExists([FileBase '.dat'])
        disp('Loading DAT...')
        Eeg = LoadBinary([FileBase '.dat'], Channels,Par.nChannels);
        eSampleRate = Par.SampleRate;
    else
    ...
    nFFT = SpecWindow*4;
    weeg = WhitenSignal(Eeg,eSampleRate*2000,2);
    [y,f,t]=mtcsglong(weeg,nFFT,eSampleRate,SpecWindow,[],2,'linear',[],FreqRange);
    t = (t(2)-t(1))/2 +t; % moved May 2nd 2018
...
if ~FileExists([FileBase '.eeg']) && FileExists([FileBase '.dat'])
    gCheckEegStates.EegFile  ='dat';
    gCheckEegStates.Channels =Channels;
    gCheckEegStates.nChannels = length(Channels);
    gCheckEegStates.nSamples = FileLength([FileBase '.dat'])/Par.nChannels/2;
else
...
AuxData=[];  % PASCALE May 16th 2012; removed input!!!
...
set(gCheckEegStates.figh, 'Name', ['CheckEegStates : ' FileBase] );
...
for ii=1:gCheckEegStates.nChannels
    ...
    imagesc(t,f,log(squeeze(y(:,:,ii)))');axis xy; ylim([max(0,FreqRange(1)) min(FreqRange(2),20)]);
    hold on
    if ii==1
        title('Spectrogram'); ylabel('Frequency (Hz)'); colormap('jet');
    end
end

if nAuxData>0
    for ii=[1:nAuxData] %#ok<NBRAK>
...
function [thratio, ThePeriods] = TDRatioAuto(y,f,t,MinLen) %#ok<DEFNU>

%automatic theta periods detection just using the thetaratio
thfin = find(f>6 & f<9);
thfout = find(f<5 | (f> 12& f<25));
thratio = log(mean(sq(y(:,thfin,1)),2))-log(mean(sq(y(:,thfout,1)),2)); %#ok<FNDSB>
...
if nargout>1
    ...
    for i=1:nStates
        thratio_st(i) = mean(thratio(TheState==i)); %#ok<AGROW>
    end

    [~, TheInd] = max(thratio_st);
...
function DisplayAuxData(Data)
        ...
        if nEl<4 
            err=1;
        elseif ~isstr(Data{4})  %#ok<DISSTR>
            err=1;
        else
            err=0;
        end
        if err 
            warning('AuxData has to be cell array where each row is : xaxis, yaxis, data, display_func'); %#ok<WNTAG>
            close 
            return;
        end
