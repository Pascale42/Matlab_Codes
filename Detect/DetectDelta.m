%function out = DetectDelta(FileBase, fMode, Channels,FreqRange)
% fMode = 'channel', 'compute', 'sort'
function out = DetectDelta(FileBase, varargin)
Par = LoadPar([FileBase '.xml']);
eFs= 1250;

[fMode, Channels, FreqRange] = DefaultArgs(varargin,{'compute',1,[100 600]});
State ='SWS';
...
switch fMode
    case 'compute'
        ...
        [out.trw.tav out.trw.tstd out.trw.twav out.trw.f out.trw.t] = ...
            TrigScalogram(eeg,Res(Burst),0.5,1250,[1 650]);
    ...
    case 'detect'
        ...
        eeg = LoadBinary([FileBase '.eeg'], Channels, Par.nChannels, [], [], 'double')';
        %csd = LoadBinary([FileBase '.csd'], Channels, Par.nChannels, [], [], 'double', Period, 1)';

        slowfeeg = ButFilter(eeg,4,10/(eFs/2),'low');
        weeg = WhitenSignal(eeg,1250*100,1);
