function [ripples,sd,bad] = FindRipples_Mae(filtered,varargin)
...
% Copyright (C) 2004-2011 by MichaÃ«l Zugaro, 2016 Ralitsa Todorova (vectorized secondPass),
% initial algorithm by Hajime Hirase
% Edited by Maëva 2019
%
...
%edited by Maëva 27/06/2019
% minInterRippleInterval = 30; % in ms 
% minRippleDuration = 20; % in ms
% maxRippleDuration = 100; % in ms
minInterRippleInterval = 30/1000*frequency; % in samples
minRippleDuration = 20/1000*frequency; % in samples
maxRippleDuration = 100/1000*frequency; % in samples
...
toMerge = iri<minInterRippleInterval & duration<maxRippleDuration;
...
% ripples(duration<minRippleDuration/1000,:) = []; % edited by Maëva 27/06/2019
ripples(duration<minRippleDuration,:) = []; % in samples
disp(['After min duration test: ' num2str(size(ripples,1)) ' events.']);

% Discard ripples that are way too long
duration = ripples(:,3)-ripples(:,1);
% ripples(duration>maxRippleDuration/1000,:) = []; % edited by Maëva 27/06/2019
ripples(duration>maxRippleDuration,:) = []; % in samples
