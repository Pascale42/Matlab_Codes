...
function out=histogramme1(FileBase,anatgrps,channels,varargin) %add anatgrpstest, channelstest
...
AllCh=[];
for n=1:nAnatGps
    AllCh=[AllCh  Par.AnatGrps(n).Channels+1];
end
clear nAnatGps

if isempty(channelstestfin)       %si channelstestfin est vide  Channelstest(2)
    if isempty(channelstest)    %si channelstest vides Channelstest(1)
...
if isempty(Channelstest)
    ...
    %addition
        ...
        lfp = LoadBinary([FileBase '.eeg'],Channels(1),Par.nChannels);
        lfpt = LoadBinary([FileBase '.eeg'],Channelstest(1),Par.nChannels); %addition lfpt
