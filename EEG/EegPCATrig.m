...
function EegPCATrig(data,Trig, FRange)

fftl= 2^10;

n_segment = length(Trig);
...
for jj=1:n_segment
  data_begin = Trig(jj)-fftl/2;
  data_end = Trig(jj)+fftl/2-1;
...
subplot(4,2,8);
plot([1:n_segment]*fftl/sr,Projected(:,4));
xlabel('time (s)');
ylabel('4th PC');

subplot(4,2,5);
plot(f_axis(axislimit),mean(N_Spectra));
