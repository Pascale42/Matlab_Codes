...
function data = LoadIntanData(FileBase, Files, channels, save, name)

% Loads the channels specified from the Intan Files
% FileBase = Root filename before '_xx.rhd'
% Files = file numbers in xx in 'FileBase_xx.rhd'
% channels = specifiy channel(s) to load
% if save ==1; saves into a FileBase.dat file
% then give 'name'

% Each Intan File is 1 minute (60.012 sec) of all channels


if nargin < 4
    disp('Not enough input arguments!!! try again!')
    return
end
...
for n=1:length(Files)
    ...
    FileName=[FileBase '_' num2str(Files(n))];
    amplifier_data = read_Intan_RHD2000_file_mod(FileName);
    
    if Files(n) == First
        data=amplifier_data;
        data=data(channels, :);
    else
        dat=amplifier_data(channels, :);
        data=[data dat];
        clear dat
    end
    ...
    disp(['     ->-> file # ' num2str(Files(n)) ' done']);
...
if save ==1
    SaveBinary( [name '.dat'], data, 'precision', 'int16', 'mode', 'new')
end

%% Dependencies

function amplifier_data = read_Intan_RHD2000_file_mod(FileBase)

% read_Intan_RHD2000_file
%
% Version 1.3, 10 December 2013
% Modified by Pascale on the May 6th 2016 (lines 21, 33, 34)
% see read_Intan_RHD2000_file_ori for original file
%
...
% PASCALE MOD: No more GUI invite to load the file
pat =FileBase;
d = dir([pat '*.rhd']);
file = d(end).name;

tic;
% filename = [path,file]; path wrongfully used % PASCALE
p=pwd;% PASCALE
filename= [p,'/',file]; % PASCALE
...
%  USAGE
% 
% SaveBinary([FileName '.dat'], VARIABLE, 'precision', 'int16', 'mode', 'new')
%
...
% Copyright (C) 2004-2010 by Micha??l Zugaro
%
% This program is free software; you can redistribute it and/or modify
% it under the terms of the GNU General Public License as published by
% the Free Software Foundation; either version 3 of the License, or
