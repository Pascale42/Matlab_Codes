...
function [T, G, Map] = LoadCluResKSNP(varargin)
...
[~,idx] = sort(Map(:,1));
Map = (Map(idx,:));
% since ref chan (192) of NP probes is deactivated for KS chan numbers
% bigger than 192 need to be corrected
Map(Map(:,3)>191, 3) = Map(Map(:,3)>191, 3) + 1;


if Save
    FileBase = gfb;
    save([FileBase '.CluRes.mat'], 'T', 'G', 'Map')
end