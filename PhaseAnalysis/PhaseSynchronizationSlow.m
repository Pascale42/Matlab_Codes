...
%function PhaseSynchronizationSlow(FileBase,fMode, ElLoc, FreqRange,State,FrThreshold,Window)
% the name is wrong - it computes, in fact, unit - lfp coherence (and other measures) for each
% channel for each cell. for slow freq.
[fMode, ElLoc, FreqRange,States,FrThreshold,Window] = DefaultArgs(varargin, ...
    {'compute','c',[1 20], {'REM','RUN','SWS'},0.5,2^12});
            ...
            for ii=1:nRepCh
                ...
                for jj=1:length(gfr);
                    ...
                    [y, out(sc).f,  phi, yerr, phierr, phloc,pow] = mtptchd(eeg,myRes,...
                        ones(length(myRes),1),2^(nextpow2(Window)+1),1250,2^nextpow2(Window),[],5,[],[],FreqRange,[],[],[],4);
                    out(sc).powf(:,ii,jj) = sq(y(:,1,1));
                    out(sc).powu(:,jj) = sq(y(:,2,2));
                    out(sc).powuf(:,ii,jj,:) = sq(pow(:,1,2,:));
