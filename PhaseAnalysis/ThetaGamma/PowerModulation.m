function out = PowerModulation(th,pow,varargin)
%function out = PowerModulation(th,pow,Shuffle)
...
% MaxShift
[Shuffle ] = DefaultArgs(varargin,{[]});
N = length(th);
powsum = sum(pow);
R = sum(exp(i*th).*pow)./powsum;
out.Ramp = abs(R);
out.Rth = angle(R);
...
phedges = linspace(-pi,pi,nbins+1); % 20 deg bins
...
if ~isempty(Shuffle) & isstruct(Shuffle) 
    ...
    for k=1:Shuffle.nShuffle
        ...
        Rsh(k) = abs(sum(exp(i*th).*pow_sh))./powsum;
