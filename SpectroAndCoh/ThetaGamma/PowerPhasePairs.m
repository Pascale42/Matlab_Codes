function [out PhMat PowMat] = PowerPhasePairs(xPh, FrBinsPh, xPow,FrBinsPow,ResampleNum,varargin)
%function [out PhMat PowMat] = PowerPhasePairs(xPh, FrBinsPh, xPow,FrBinsPow,Resample, FunHandle, Args)
...
%             end
            %---------------------------------------------
            frombutt=ButFilter(xPh, 2, FrBinsPh(kPh)*ResampleNum, 'bandpass');
            ph = angle(hilbert(frombutt));clear frombutt
            if UniPhase
                for kPhCh=1:nPhChan
