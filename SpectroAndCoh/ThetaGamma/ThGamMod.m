    ...
    case 'compute'
        ...
        for w = 1:length(States) %loop through states
            ...
            if ThBins==0
                ...
                if LoadAll==1
                    eeg = LoadBinary([FileBase '.lfp'],Channels, Par.nChannels, 3,[],[],Period)';
                    out(wcnt).thgam = PowerPhasePairs(ThPh, 0, eeg,  GamBins/625,10,@PowerModulation,Shuffle);
                else
                    loadChannel = @(ChInd) LoadBinary([FileBase '.lfp'],Channels(ChInd), Par.nChannels, 4,[],[],Period)';
                    eeg = struct('FunHandle',loadChannel,'nPow',length(Channels));
                    out(wcnt).thgam = PowerPhasePairs(ThPh,0,eeg ,  GamBins/625,10,@PowerModulation,Shuffle);
                end
            else
                if LoadAll==1
                    eeg = LoadBinary([FileBase '.eeg'],Channels, Par.nChannels, 3,[],[],Period)';
                    out(wcnt).thgam = PowerPhasePairs(eeg(:,PhChannelInd), ThBins/625, eeg,  GamBins/625,10,@PowerModulation,Shuffle);
                else
                    loadChannel = @(ChInd) LoadBinary([FileBase '.eeg'],Channels(ChInd), Par.nChannels, 3,[],[],Period)';
                    eeg = struct('FunHandle',loadChannel,'nPow',length(Channels));
