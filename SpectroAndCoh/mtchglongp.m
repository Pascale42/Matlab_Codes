    ...
    for Ch1=1:nChannels, for Ch2 = Ch1:nChannels
            ...
            if Ch1==Ch2
                if length(t)==1
                    ...
                    imagesc(t,f,20*log10(abs(y(:,:,Ch1,Ch2))+eps)');axis xy; colormap(jet); axis off; %colorbar
                end
%                 title(['Power specgram ' num2str(Ch1)]);
            else
                %imagesc the coherogram
                imagesc(t,f,(abs(y(:,:,Ch1,Ch2)))');axis xy; colormap(jet);axis off;%colorbar
%                 title(['Coherogram ' num2str(Ch1)]); 
                
                
                %display phaseogram
                subplot(nChannels, nChannels, Ch2 + (Ch1-1)*nChannels);
                imagesc(t,f,squeeze(phi(:,:,Ch1,Ch2))');axis xy; colormap(jet);axis off;%colorbar
