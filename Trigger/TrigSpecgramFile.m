% function [Spec, f, t] = TrigSpecgramFile(FileBase, Trigger, Channels, Window, SpecWin, SpecStep ,Fs, FreqRange, Whiten, Tapers, Acc, ResampleCoef, DoCSD)
% Window, SpecWindow and SpecStep are in msec
% Trigger in sampling rate of the file
function [spec, f, t] = TrigSpecgramFile(FileBase, Trigger,varargin)

Par =LoadPar([FileBase]);
[Channels, Window, SpecWin, SpecStep ,Fs, FreqRange, Whiten, Tapers, Acc, ResampleCoef, DoCSD] = DefaultArgs(varargin,...
{[1:16],  1000,    200,     100,     1250, [1 100] , 1, [3 5],0,1 ,0});
if ResampleCoef >1 
...
segs = LoadSegs([FileBase '.eeg'], StartPoints,nSamples,Par.nChannels,Channels,Fs,ResampleCoef,1); 
