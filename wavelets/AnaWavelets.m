function [wt,freqlist,psi_array] = AnaWavelets(x,Fs,freqlist,type,xi,display)
%
%   awt_freqlist  analytical wavelet transform, where one can specify the list of desired frequencies 
%   
%   [wt,freqlist,psi_array] = AnaWavelets(x,Fs,freqlist,type,xi,display)
%
%   Inputs:
...
% 7/02/2012: (ppq) called AnaWavelets (awt_freqlist before) and added plotting

% 
% [wt,freqlist,psi_array] = AnaWavelets(dat([1:(32556*20)],3),32556,[0.5:0.1:10],'Gabor',5,0);
% figure;  imagesc([1:size(wt,1)],freqlist,abs(wt)'); axis xy
...
if display == 1
   ...
   subplot 212; imagesc([1:size(wt,1)],freqlist,abs(wt)'); axis xy; % coloration('dark');
